...
namespace Zly\Doctrine\Translatable\Query\TreeWalker;

use Zly\Doctrine\Translatable\Mapping\Event\Adapter\ORM as TranslatableEventAdapter;
use Zly\Doctrine\Translatable\TranslationListener;
 ...
 * @package Zly\Doctrine.Translatable.Query.TreeWalker
...
class TranslationWalker extends SqlWalker
{
    ...
    public function getExecutor($AST)
    {
        if (!$AST instanceof SelectStatement) {
            throw new \Zly\Doctrine\Exception\UnexpectedValueException('Translation walker should be used only on select statement');
        }
    ...
    public function walkSelectStatement(SelectStatement $AST)
    {
        ...
        if ($this->needsFallback()) {
            ...
            if ($hydrationMode === Query::HYDRATE_ARRAY) {
                ...
                $this->getEntityManager()->getConfiguration()->addCustomHydrationMode(
                    ...
                    'Zly\Doctrine\\Translatable\\Hydrator\\ORM\\ArrayHydrator'
                );
            }
        }
        ...
        if ($hydrationMode === Query::HYDRATE_OBJECT) {
            ...
            $this->getEntityManager()->getConfiguration()->addCustomHydrationMode(
                ...
                'Zly\Doctrine\\Translatable\\Hydrator\\ORM\\ObjectHydrator'
            );
            ...
            $this->getEntityManager()->getConfiguration()->addCustomHydrationMode(
                ...
                'Zly\Doctrine\\Translatable\\Hydrator\\ORM\\SimpleObjectHydrator'
            );
            $this->getQuery()->setHint(Query::HINT_REFRESH, true);
        }
        return $result;
     ...
     *
     * @throws \Zly\Doctrine\Exception\RuntimeException - if listener is not found
    ...
    private function getTranslationListener()
    {
        ...
        if (is_null($translationListener)) {
            throw new \Zly\Doctrine\Exception\RuntimeException('The translation listener could not be found');
        }
        return $translationListener;
    }
}