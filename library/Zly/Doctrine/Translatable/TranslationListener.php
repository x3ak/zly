...
use Doctrine\Common\EventArgs,
    ...
    Zly\Doctrine\Mapping\MappedEventSubscriber,
    Zly\Doctrine\Translatable\Mapping\Event\TranslatableAdapter;
...
class TranslationListener extends MappedEventSubscriber
{
    ...
    public function getTranslatableLocale($object, ClassMetadata $meta)
    {
        ...
        if (isset($this->configurations[$meta->name]['locale'])) {
            ...
            if (!$reflectionProperty) {
                ...
                throw new \Zly\Doctrine\Exception\RuntimeException("There is no locale or language property ({$column}) found on object: {$meta->name}");
            }
    ...
    protected function validateLocale($locale)
    {
        if (!is_string($locale) || !strlen($locale)) {
            throw new \Zly\Doctrine\Exception\InvalidArgumentException('Locale or language cannot be empty and must be set through Listener or Entity');
        }
    }
