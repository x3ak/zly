...
namespace Zly\Doctrine\Tree\Strategy\ORM;

use Zly\Doctrine\Tool\Wrapper\EntityWrapper;
use Zly\Doctrine\Tool\Wrapper\AbstractWrapper;
use Zly\Doctrine\Tree\Strategy,
    Doctrine\ORM\EntityManager,
    Zly\Doctrine\Tree\TreeListener,
 ...
 * @package Zly\Doctrine.Tree.Strategy.ORM
 * @subpackage Nested
...
class Nested implements Strategy
{
    ...
    public function setNodePosition($oid, $position)
    {
        ...
        if (!in_array($position, $valid, false)) {
            throw new \Zly\Doctrine\Exception\InvalidArgumentException("Position: {$position} is not valid in nested set tree");
        }
    ...
    public function processScheduledUpdate($em, $node)
    {
        ...
        if (isset($config['root']) && isset($changeSet[$config['root']])) {
            throw new \Zly\Doctrine\Exception\UnexpectedValueException("Root cannot be changed manualy, change parent instead");
        }
     ...
     * @param string $position
     * @throws Zly\Doctrine\Exception\UnexpectedValueException
     * @return void
     */
    public function updateNode(EntityManager $em, $node, $parent, $position = 'FirstChild')
    {
        ...
        if ($parent) {
            ...
            if (!$isNewNode && $rootId === $parentRootId && $parentLeft >= $left && $parentRight <= $right) {
                throw new \Zly\Doctrine\Exception\UnexpectedValueException("Cannot set child as parent to node: {$nodeId}");
            }
            if (isset($config['level'])) {
