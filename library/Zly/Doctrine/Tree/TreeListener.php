...
class TreeListener extends MappedEventSubscriber
{
    ...
    public function getStrategy(ObjectManager $om, $class)
    {
        if (!isset($this->strategies[$class])) {
            ...
            if (!$config) {
                throw new \Zly\Doctrine\Exception\UnexpectedValueException("Tree object class: {$class} must have tree metadata at this point");
            }
            ...
            if (!isset($this->strategyInstances[$config['strategy']])) {
                ...
                if (!class_exists($class)) {
                    throw new \Zly\Doctrine\Exception\InvalidArgumentException($managerName." TreeListener does not support tree type: {$config['strategy']}");
                }
