...
namespace Zly\Doctrine\Tree\Entity\Repository;

use Zly\Doctrine\Tool\Wrapper\EntityWrapper;
use Doctrine\ORM\Query,
    Zly\Doctrine\Tree\Strategy,
    Zly\Doctrine\Tree\Strategy\ORM\Nested,
    Zly\Doctrine\Exception\InvalidArgumentException,
 ...
 * @package Zly\Doctrine.Tree.Entity.Repository
...
class NestedTreeRepository extends AbstractTreeRepository
{
    ...
    public function __call($method, $args)
    {
        if (substr($method, 0, 9) === 'persistAs') {
            if (!isset($args[0])) {
                throw new \Zly\Doctrine\Exception\InvalidArgumentException('Node to persist must be available as first argument');
            }
            $node = $args[0];
            ...
            if (substr($method, -2) === 'Of') {
                if (!isset($args[1])) {
                    throw new \Zly\Doctrine\Exception\InvalidArgumentException('If "Of" is specified you must provide parent or sibling as the second argument');
                }
                $parent = $args[1];
     ...
     * @throws \Zly\Doctrine\Exception\InvalidArgumentException - if input is invalid
     * @return Query
     */
    ...
    public function removeFromTree($node)
    {
        ...
        if ($node instanceof $meta->name) {
            ...
            try {
                ...
                throw new \Zly\Doctrine\Exception\RuntimeException('Transaction failed', null, $e);
            }
        } else {
            throw new InvalidArgumentException("Node is not related to this repository");
        }
